...
# correlation(df[vars], select(df, starts_with("Item_IAS"))) |> 
#   summary() |> 
#   plot()
```
...
make_correlation <- function(x, y) {
  ...
  if(nrow(r) == 0) list(data = cor, print = "No significant (BF >= 10) correlations.")
...
bpd <- make_correlation(df[vars], select(df, starts_with("BPD_"))) 
...
age <- make_correlation(df[vars], select(df, Age)) 
...
p_cor <- rbind(
  ...
  mutate(alpha = ifelse(BF >= 10, "BF >= 10", "BF < 10"),
         ...
         Domain = fct_relevel(Domain, "Normal Personality", "Maladaptive Personality", "Autistic Traits", "Schizotypic Traits"),
         Parameter2 = str_remove_all(Parameter2, "IPIP6_|PID5_|ASQ_|SPQ_")) |>
  ...
  scale_fill_gradient2(low = "#2196F3", high = "#FF5722") +
