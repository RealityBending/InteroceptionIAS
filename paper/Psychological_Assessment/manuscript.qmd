...
impact-statement: "This study analyzed data from over 33,000 people to evaluate a widely used measure of how accurately individuals perceive internal bodily signals. The findings show that this ability varies across different bodily systems and relates differently to psychological traits, improving how researchers assess interoception and understand its role in mental and physical health."
floatsintext: true
numbered-lines: true
...
language:
  ...
  title-conflict-of-interest: "Conflict of Interest."
...
Interoception refers to the process of sensing, interpreting and integrating information of internal bodily stimuli by the nervous system, including both internal organs (e.g., heart, lungs, gut) but also more broadly physiological tissues, providing information about the body's current state [@khalsa2018interoception]. Although research emphasizes interoception's fundamental role in numerous  processes, such as emotion recognition [@terasawa2021effects] and regulation [@zamariola2019relationship], decision making [@pollatos2023interoceptive], learning [@joshi2023interoception], body-ownership [@raimo2021body] and outcomes including physical [@harrison2024interoception] and psychological health [@nord2021disrupted] along with well-being [@ferentzi2019body], the field remains hampered by concerns regarding both the conceptualization and measurement of interoception [@desmedt2025discrepancies; @desmedt2022measures; @murphy2024interoception]. 

## The Interoceptive Assessment Puzzle

<!-- Interoception measure -->

Given the central nature of interoceptive processing to brain function, many measures could be seen as "indirectly" assessing (whether intentionally or not) interoceptive processes. This includes indices that quantify the coupling between physiological signals and cognitive or behavioural processes (e.g., Heart Rate Variability), or constructs that theoretically depend on interoceptive information (e.g., alexithymia). Critically, various *direct* measures of interoception have been developed (see @fig-measures), forming a combination of "objective" and "subjective" assessments (i.e., physiological tasks such as the heart beat counting or tracking *vs.* questionnaires and subjective scales involving metacognitive judgments), "explicit" and "implicit" paradigms (i.e., directing participants' awareness and attention to interoceptive processes *vs.* measuring interoception covertly), various interoceptive modalities (e.g., cardioception, respiroception, gastroception) and theoretical dimensions (e.g., accuracy, sensitivity, awareness). 
...
The diversity in measurement approaches illustrates that rather than there being a "gold-standard" measure of interoception in the field, each measure likely has strengths and limitations depending on the research context and dimension of interoception being assessed [@jahedi2014; @Desmedt2023accuracy].
...
<!-- self-reports -->
Although using self-report questionnaires to measure deeply embodied functions might seem paradoxical, recent redefinitions of interoception emphasize the importance of high-level and metacognitive elaboration of interoceptive information, and support the idea that some facets of interoception, in particular participants' metacognitive beliefs, can be assessed subjectively [@khalsa2018interoception; @suksasilp2022towards].
The notion that self-reported interoception reflects a fundamentally different aspect of conscious interoceptive processing compared to objective task measures is thought to be central to understanding the reported lack of convergence between objective and subjective measures [@desmedt2022measures]. For instance, there are typically weak or no correlation between questionnaires and objective measures [HCT, @schandry1981heart] and the Heartbeat Detection Task [HDT, @kleckner2015methodological], even when both measures are intended to capture the same theoretical interoceptive dimension [e.g., task-based accuracy vs. self-reported accuracy, @murphy2019; @brand2023; @arslanova2022]. 
Perhaps more surprisingly, there is also a reported lack of convergence amongst objective or subjective measures purporting to measure the same interoceptive dimension. For instance, task-based measures targetting accuracy typically show no or weak correlation amongst themselves [respectively, @brand2023; @hickman2020relationship], and subjective scores of interoceptive accuracy also yield weaker than expected relationships [@vig2022questionnaires]. As well as posing substantial validity concerns, these findings raise the possibility that even within broad measurement domain, different measures are targeting fundamentally different facets of interoception, undermining replicability and hindering theoretical and empirical progress in the field.


One striking example concerns the assessment of interoceptive sensibility, which is defined as the self-reported tendency to focus on (with or without correctly detecting) internal sensations [@garfinkel2015knowing; @khalsa2018interoception].
...
Given increasing interest in interoception across psychology and neuroscience, and the growing use of the IAS, careful scrutiny of this novel self-report measure of interoceptive accuracy is warranted. Therefore, the current study has two aims. First (study 1), to clarify the structure of the IAS using a mega-analytic approach which leverages existing data by aggregating existing datasets and re-analysing them at the raw data level. This aggregated data will then be used to contrast the traditional CFA/SEM factor-based analyses and Hierarchical Clustering Analysis (HCA) with newer network-based ones (Exploratory Graph Analysis - EGA). By triangulating our results across different samples and conceptually distinct analytical frameworks, the current paper aims to provide a more nuanced and method-independent view of the IAS factor structure, allowing the identification of both convergent and potentially overlooked structural features that may not be apparent when relying on a single analytic approach. The second aim (study 2) is to provide an overview of construct validity and the dispositional correlates of the IAS, clarifying its general pattern of associations with interoception-related constructs, mood, psychopathological and neurodevelopmental traits and other relevant measures and thereby situating interoception questionnaires within a broader context.
...
This study’s design and its analysis were not preregistered. However, all data, study materials, and code used in this analysis are publicly available at the [MASKED FOR REVIEW] and can be accessed at [MASKED FOR REVIEW]. Data was analysed using R, version 4.5.2 and the EGAnet package version 2.4.0 [@christensen2021estimating], the pvclust package version 2.2-0 [@suzuki2015package], the the easystats R ecosystem version 0.7.5 [@ben2020effectsize; @ludecke2020extracting; @makowski2025modelbased], the laavan package version 0.6-21 [@rosseel2012lavaan], the tidyverse library version 1.1.11 [@wickham2017package], and the tidySEM package version 0.2.10 [@van2024recommended]. 
...
All data, study materials, and code used in this analysis are publicly available at the [MASKED FOR REVIEW] and can be accessed at [MASKED FOR REVIEW]. Data was analysed using R, version 4.5.2 and the easystats R ecosystem version 0.7.5 [@ben2020effectsize; @ludecke2020extracting; @makowski2025modelbased], the tidyverse library [@wickham2017package], and the *glmmTMB* package version 1.1.11 [@glmTMB]. 
...
Akin to a meta-analytic framework, datasets were treated as clusters (random groups), and pooled estimates of the associations that account for both within- and between-sample variability were computed. This approach allowed for the estimation of multiple associations while still accounting for study heterogeneity and intra-study uncertainty (i.e., measurement uncertainty within the studies). Specifically, associations were estimated using mixed models. Each predictor was entered separately with the seven IAS item pairs identified in Study 1 (Hungry–Thirsty, Muscles–Pain, Wind–Burp, Urinate–Defecate, Breathing–Heart, Bruise–Blood sugar, Cough–Sneeze) and the original IAS "total" score serving as outcomes. Models included dataset as a random intercept and the predictor as a random slope. Note that given the large sample size and iterative hypothesis testing, we focus on effect sizes and confidence intervals rather than *p*-values which are not relevant for inference in this context. 

<!-- When models failed to converge, a reduced structure with random intercepts only will be used.  -->

<!-- Extraction of parameters -->
From each fitted model, we extracted standardized regression coefficients (interpretable as a correlation index), and 95\% confidence intervals (additional details on *p*-values, convergence status, and sample sizes are available in the link above). Results are summarized in @fig-correlations and we report the mean coefficient in the text when consistent across measures. 
...
The present findings should be interpreted considering the composition of the aggregated datasets. Although the large sample (N > 33,000) spans multiple countries and languages, it does not represent a clearly defined population, but rather a collection of convenience samples with varying recruitment strategies. Participants were predominantly from Western populations and disproportionately female (~71%), limiting generalizability to more diverse demographic and cultural groups. Additionally, heterogeneity in language, measurement formats, and study procedures across datasets may influence both the structure of the IAS and its associations with external variables. As such, the findings are best understood as generalizing to populations typically studied in psychological research rather than the broader global population. Future work should examine the robustness of these results in more diverse and systematically sampled populations.
