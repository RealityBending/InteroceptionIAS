...
All variables from the 16 open-access datasets will be extracted and standardized (z-scored). For continuous predictors, we will first ensure consistent ranges across datasets; when necessary, ranges will be rescaled to achieve comparability. In cases where scales of different lengths measured the same construct, scores were harmonized: when possible, means were computed from overlapping items, while in other cases total scores were normalized by the number of items. Redundant subscales (e.g., PHQ-2 and GAD-2 within PHQ-4) were collapsed into unified composite scores. Similarly, short forms of trait anxiety (e.g., STAI-5-trait) were collapsed with their longer counterpart (STAI-T), with scores computed as the mean to ensure comparable ranges. Longer formats of the PI (PI-99) were collapsed into its shorter version (PI-18). Missing data were handled via listwise deletion on a per-model basis prior to fitting.

<!-- Multilevel modelling approach -->
Associations were estimated using multilevel regression models implemented in the glmmTMB package [@glmmTMB]. Each predictor was entered separately, with the seven IAS item pairs identified in Study 1 (“Hungry–Thirsty”, “Muscles–Pain”, “Wind–Burp”, “Urinate–Defecate”, “Breathing–Heart”, “Bruise–Blood”, “Cough–Sneeze”) and the original IAS scale serving as outcomes. Models included random intercepts and slopes by sample to account for between-study heterogeneity; when models failed to converge, a reduced structure with random intercepts only was used. This approach treats datasets as clusters, thereby providing pooled estimates of the associations while accounting for both within- and between-sample variability, akin to a meta-analytic framework.

<!-- Extraction of parameters -->
From each fitted model, we extracted standardized regression coefficients (β), 95% confidence intervals, p-values, convergence status, and effective sample size. Participants identifying as “Other” were excluded from analyses due to limited representation.


## Results

<!-- Demographic correlates -->
Age showed consistent positive associations across tasks (mean r = .12), with all correlations significant. Gender differences were weaker (mean r = −.01), though a majority of effects reached significance.

<!-- Interoceptive measures -->
Several MAIA subscales showed moderate and consistently significant positive associations with IAS scores. Noticing (mean β = .259), Attention Regulation (β = .249), Trusting (β = .236), Emotional Awareness (β = .197), Listening (β = .176), and Self-Regulation (β = .189) were significant across all pairings and the original scale. Autonomic reactivity from the BPQ was negatively associated (β = −.173, 88% significant), whereas body awareness showed weaker and less reliable effects (β = .122, 25% significant). Alexithymia was consistently negative: ICQ (β = −.316), TAS subscales: Difficulties Identifying Feelings (βs = −.211) , Difficulties Describing Feelings (βs = −.150), External Thinking (βs = −.081, 75% significance), and the BVAQ cognitive component (β = −0.200) were significant for nearly all IAS correlates. 

<!-- Mood questionnaires -->

Depression (BDI: β = −0.204) and anxiety (STAI-T: β = −0.189) showed reliably negative associations. PHQ-15 (β = −0.128, 75% significant) PHQ-4 (depression: β = −0.094, 75% significant; anxiety: β = −0.103, 88% significant), and GAD-7 (β =−0.187, 63% significance) also showed consistent negative effects. By contrast, PHQ-9 (β = −0.619) and MFQ (β = −0.139) were largely nonsignificant despite negative averages.

<!-- Psychopathology and personality -->

Autistic traits showed the clearest pattern: Imagination (β = −0.184, 88% significant), Low Attentional Switching (β = −0.143, 63% significant), and Lack of Social Skills (β = −0.119, 63% significant) were reliably negative. Among maladaptive traits, Negative Affect (β = −0.111, 63% significant) was the only consistent correlate. Schizotypal traits showed generally weak and unreliable associations, with No Close Friends (β = −0.12, significant in 50% of models) being the only subscale to exhibit a small effect, and even this just at the threshold of consistency.

<!-- Personality and beliefs -->

Of the Big Five, only Openness showed consistent positive associations (β = 0.123, 63% significant). Other traits were weak or mostly nonsignificant. Conspiracy, world belief, and lying profiles showed no reliable associations.

[**ANA: Confirm personality**]
