...
The first study aimed to clarify the factor structure of the IAS, a widely used subjective measure of interoception whose dimensionality remains contested. Previous validation and psychometric studies have proposed conflicting models ranging from a single general factor [@brand2023] to bifactor [@campos2021; @lin2023], and two-factor solutions [@murphy2019; @koike2023]. At the same time, network-based approaches such as EGA emphasize local dependencies between items, identifying clusters at the item level that may not map neatly onto broader latent constructs [@christensen2023; @desmedt2022measures]. By leveraging open-access datasets and reanalyzing them both in aggregate and individually using a mega-analytic approach, and by contrasting traditional factor-analytic models with network-based methods, this study provides the most comprehensive test of the IAS structure to date.

<!-- 7-pairs of items as optimal model -->

Overall, the findings converge on a robust yet novel picture. Exploratory and confirmatory analyses indicated that the IAS is best characterized by seven pairs of closely related items, consistently identified by EGA and supported by HCA. These pairs (e.g., hunger–thirst, urinate–defecate, cough–sneeze) were highly replicable across independent samples and yielded superior fit indices in CFA compared to one-factor, three-factor, three-factor with ageneral factor, five-factor, a five-factor with a general factor, a 7-factor or a seven factor with a general factor alternatives. Although the one-factor model occasionally achieved lower BIC, Bayes factor comparisons overwhelmingly supported the EGA structure, underlining its empirical robustness.

<!-- IAS not higher-order, but item-level dependencies -->

This pattern has several important implications. First, it suggests that the IAS does not capture a broad latent construct of interoceptive accuracy as originally intended, but rather reflects a set of tightly coupled item clusters tied to specific bodily sensations. In other words, the IAS may function more as a checklist of bodily signal detection items than as a scale measuring a coherent higher-order dimension. This stands in contrast to the original two-factor proposal [@murphy2019], where the factors were highly correlated and often collapsed into a unidimensional score, and helps explain why subsequent studies diverged in their conclusions: the psychometric structure of the IAS appears to be driven more by local dependencies than by robust latent factors. Second, the presence of redundant or unstable items (e.g., tickle, taste, affective touch), already highlighted in prior validation studies [@campos2021; @lin2023], undermines the interpretability of the scale, as their contributions appear inconsistent or overlapping. Together, these findings help explain why previous research has struggled to converge on a stable factor structure: the IAS lacks the psychometric properties required for strong latent modeling and may overstate its dimensionality by conflating redundancy with structure.

**[ADD THAT LIN CREATED A SHORT VERSION OF THE IAS??]**

<!-- Improvements for the scale + proposal of a new scale-->

At the same time, the results do not render the IAS unusable. The consistent emergence of item pairs highlights intuitive bodily domains (e.g., visceral, expulsion, musculoskeletal) that align with everyday interoceptive experiences, supporting its ecological validity. However, the limited breadth of these pairings, the redundancy across items, and the reliance on a narrow 5-point scale all restrict the scale’s ability to differentiate between individuals and to capture the multidimensional nature of interoception [@desmedt2022measures]. These findings therefore call for refinement—either through targeted revisions of the IAS (e.g., removing redundant items, expanding modality coverage, adopting analogue response formats) or through the development of a new questionnaire that better balances theoretical coherence with empirical validity.

<!-- Summary -->

In sum, Study 1 demonstrates that the IAS structure is best described by clusters of tightly coupled item pairs reflecting local dependencies, rather than by a broad latent dimension. While this solution is replicable and statistically robust, it highlights the conceptual and psychometric limitations of the IAS in its current form. These insights help contextualize the mixed findings on the scale’s validity and correlations with other interoceptive measures [@murphy2019; @brand2023; @lin2023] and provide a crucial foundation for Study 2, where we  to the dispositional correlates of the IAS to further evaluate its construct validity and situate it within the broader network of interoceptive and psychological traits.
...
**Interoception Related**
The Multidimensional Assessment of Interoceptive Awareness Version-2 [MAIA-2, @memehling2018multidimensional] is a 37-item questionnaire assessing eight dimensions of interoception: Noticing, Not-Distracting, Not-Worrying, Attention Regulation, Emotional Awareness, Self-Regulation, Body Listening, and Trust. Responses are rated on a 6-point Likert scale (e.g., “I notice when I am uncomfortable in my body”).
...
The State–Trait Anxiety Inventory Trait Version [STAI-T, @spielberger1970manual] is a 20-item trait anxiety questionnaire rated on a 4-point Likert scale (e.g., “I worry too much about something that really doesn’t matter”).
...
<!-- Personality -->
**Personality**
The NEO Five-Factor Inventory, neuroticism subscale [NEO-FFI, @costa1989neoffi] is a 12-item measure of neuroticism, assessing the general tendency to experience negative emotions such as fear, sadness, or disgust. Items are rated on a 5-point Likert scale (e.g., “I often feel inferior to others”).
...
The Autism Spectrum Quotient Short Form [ASQ-S, @Hoekstra et al., 2011] is a 28-item measure of five autistic traits: social skills, adherence to routines, cognitive flexibility, imagination, and patterns/numbers. Items are rated on a 4-point Likert scale (e.g., “I find it difficult to work out people’s intentions”).
...
The Primal World Beliefs Inventory Short Form [PI-18, @lifton2021brief] is an 18-item measure assessing four high-order beliefs: the world is Good (vs. Bad) and its three dimensions: Safe (vs. Dangerous), Enticing (vs. Dull), and Alive (vs. Mechanistic). Items are scored on a 5-point Likert scale (e.g., “Nearly everything in the world is beautiful”). The PI-99 [@clifton2019primal] scores were combined with the 4 high-order beliefs of the PI-18. 

The Structure of Deception [SoD, @makowski2023structure] is a 16-item measure of lying tendencies across four components: Ability, Frequency, Negativity, and Contextuality. Items are scored on visual analogue scales (e.g., “I am a good liar”).

### Data Analysis

<!-- Preprocessing and standardization -->

All variables from the 16 open-access datasets will be extracted and standardized (z-scored). For continuous predictors, we will first ensure that their ranges are consistent across datasets; when necessary, ranges will be rescaled to achieve comparability. In cases where scales of different lengths measure the same construct, scores will be harmonised: when possible, means will be computed from overlapping individual items, while in other cases, total scores will be normalized by the number of items. Where redundant subscales exist (e.g., PHQ-2 and GAD-2 items contained within the PHQ-4), these will be collapsed into unified composite scores (e.g., PHQ-4 depression and PHQ-4 anxiety). Similarly, short forms of trait anxiety measures (e.g., STAI-5-trait) will be collapsed with their longer counterparts (STAI-T), with scores computed as the mean to ensure comparable ranges despite differing item counts. Additionally, longer formats of the PI (32 and 99 items) will be collapsed into its shorter versions PI-18. Missing data will be handled via listwise deletion prior to model fitting.

<!-- Multilevel modelling approach -->

Associations will be estimated using multilevel regression models implemented in the glmmTMB package [@glmmTMB]. Each predictor will be entered separately, with IAS pair of ititems identified in Study 1 (i.e., "Hungry–Thirsty", "Muscles–Pain", "Wind–Burp", "Urinate–Defecate", "Breathing–Heart", "Bruise–Blood", "Cough–Sneeze") and the original IAS scale serving as outcomes. Models will include random intercepts and slopes by sample to account for between-study heterogeneity; when models fail to converge, a reduced structure with random intercepts only will be used. This approach will treat datasets as clusters, thereby providing a meta-analytic estimate of the pooled associations while simultaneously accounting for both within- and between-sample variability, an advantage over simple pooled correlations.

<!-- Extraction of parameters -->

From each fitted model, we will extract standardized regression coefficients (r), 95% confidence intervals, p-values, convergence status, and effective sample size. Gender will be modeled as a binary factor (Female vs. Male), with participants identifying as “Other” excluded from analyses due to limited representation.
