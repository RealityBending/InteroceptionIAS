...
df11  <- read.csv("https://raw.githubusercontent.com/DominiqueMakowski/PHQ4R/main/study2/data/data.csv") |>
  ...
  mutate(across(starts_with("MAIA2_"), \(x) datawizard::rescale(x, to = c(0, 5))))  |>
   mutate(across(starts_with("PHQ4_Depression_"), \(x) datawizard::rescale(x, to = c(0, 4))),
          across(starts_with("PHQ4_Anxiety_"), \(x) datawizard::rescale(x, to = c(0, 4)))) |>
   mutate(PHQ4_Depression = (PHQ4_Depression_3 + PHQ4_Depression_4)/2,
          PHQ4_Anxiety = (PHQ4_Anxiety_1 + PHQ4_Anxiety_2)/2) |>
  datawizard::rescale(select=ias_vars, to = c(1, 5), range = c(0, 1))
