...
df2 <- data |> 
  ...
  rename(
    ...
    MAIA_NonDistracting=MAIA_NotDistracting, MAIA_NotWorrying=MAIA_NotWorrying,
...
df4$MAIA_AttentionRegulation <- rowMeans(select(df4, starts_with("MAIA_AR")), na.rm = TRUE)
...
df4$MAIA_EmotionalAwareness <- rowMeans(select(df4, starts_with("MAIA_EA")), na.rm = TRUE)
df4$MAIA_SelfRegulation <- rowMeans(select(df4, starts_with("MAIA_SR")), na.rm = TRUE)
df4$MAI_Noticing < rowMeans(select(df4, starts_with("MAIA_NT")), na.rm = TRUE)
```
...
df7a <- haven::read_sav("../data/Brand2023/Data_Giessen.sav") |> 
  ...
  rename(
    ...
    Muscles = IAS_15, Bruise = IAS_16, Pain = IAS_17, Blood_Sugar = IAS_18,
    Affective_touch = IAS_19, Tickle = IAS_20, Itch = IAS_21,
    MAIA_Noticing = MAIA_NOTICING, MAIA_NonDistracting = MAIA_NON_DISTRACTING,
    MAIA_NotWorrying = MAIA_NOT_WORRYING, MAIA_AttentionRegulation = mAIA_ATTENTION_REGULATION,
    MAIA_SelfRegulation = MAIA_SELF_REGULATION, MAIA_BodyListening = MAIA_BODY_LISTENING,
    MAIA_Trusting = MAIA_TRUSTING, MAIA_EmotionalAwareness = MAIA_EMOTIONAL_AWARENESS
  )
...
df7b <- haven::read_sav("../data/Brand2023/Data_Mainz.sav") |> 
  ...
  rename(
    ...
    Muscles = IAS_15, Bruise = IAS_16, Pain = IAS_17, Blood_Sugar = IAS_18,
    Affective_touch = IAS_19, Tickle = IAS_20, Itch = IAS_21,
    MAIA_Noticing = maiaNotice, MAIA_NonDistracting = maiaNonDistracting,
    MAIA_NotWorrying = maiaNotWorrying, MAIA_AttentionRegulation = maiaAttentionRegulation,
    MAIA_SelfRegulation = maiaSelfRegulation, MAIA_BodyListening = maiaBodyListening,
    MAIA_Trusting = maiaTrusting, MAIA_EmotionalAwareness = maiaEmotionalAwareness
  )
...
df7c <- haven::read_sav("../data/Brand2023/Data_PotVie.sav") |> 
  ...
  rename(
    ...
    MAIA_Noticing = MAIA_noticing, MAIA_NonDistracting = MAIA_notdis, 
    MAIA_NotWorrying = MAIA_notwor, MAIA_AttentionRegulation = MAIA_attreg,
    MAIA_SelfRegulation = MAIA_selfreg, MAIA_BodyListening = MAIA_bodlis,
    MAIA_Trusting = MAIA_trust, MAIA_EmotionalAwareness = MAIA_emoaw,
    ICQ = ICQ_sum
  )
```
...
df10b$MAIA_AttentionRegulation <- rowMeans(select(df10b, starts_with("MAIA2_Attention")))
df10b$MAIA_EmotionalAwareness <- rowMeans(select(df10b, starts_with("MAIA2_Emotional")))
df10b$MAIA_Noticing <- rowMeans(select(df10b, starts_with("MAIA2_Noticing")))
df10b$MAIA_BodyListening <- rowMeans(select(df10b, starts_with("MAIA2_BodyListening")))
df10b$MAIA_NotDistracting<- rowMeans(select(df10b, starts_with("MAIA2_NotDistracting")))
df10b$MAIA_NotWorrying<- rowMeans(select(df10b, starts_with("MAIA2_NotWorrying")))
df10b$MAIA_SelfRegulation<- rowMeans(select(df10b, starts_with("MAIA2_SelfRegulation")))
df10b$MAIA_Trusting<- rowMeans(select(df10b, starts_with("MAIA2_Trusting")))
