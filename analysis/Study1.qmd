...
df2 <- data |> 
  ...
  rename(
    ...
    MAIA_AttentionRegulation = MAIA_AttentionReg, MAIA_BodyListening = MAIA_BodyListening,
    MAIA_Trusting = MAIA_BodyTrusting, MAIA_EmotionalAwareness = MAIA_EmoAwareness,
    MAIA_NotDistracting=MAIA_NotDistracting, MAIA_NotWorrying=MAIA_NotWorrying,
    MAIA_SelfRegulation=MAIA_SelfReg, MAIA_Noticing=MAIA_Noticing)
rm(data)
```
...
df4$MAIA_BodyTrusting <- rowMeans(select(df4, starts_with("MAIA_TR")), na.rm = TRUE)
...
df11  <- read.csv("https://raw.githubusercontent.com/DominiqueMakowski/PHQ4R/main/study2/data/data.csv") |>
  ...
  mutate(
    MAIA_NotDistracting_1_R = 1 - MAIA2_NotDistracting_1_R,
    MAIA_NotDistracting_2_R = 1 - MAIA2_NotDistracting_2_R,
    MAIA_NotDistracting_3_R = 1 - MAIA2_NotDistracting_3_R,
    MAIA_NotDistracting_4_R = 1 - MAIA2_NotDistracting_4_R,
    MAIA_NotDistracting_5_R = 1 - MAIA2_NotDistracting_5_R,
    MAIA_NotDistracting_6_R = 1 - MAIA2_NotDistracting_6_R,
    MAIA_NotWorrying_1_R = 1 - MAIA2_NotWorrying_1_R,
    MAIA_NotWorrying_2_R = 1 - MAIA2_NotWorrying_2_R,
    MAIA_NotWorrying_5_R = 1 - MAIA2_NotWorrying_5_R)
...
df13 $MAIA_AttentionReg <- rowMeans(select(df13 , starts_with("MAIA2_Attention")))
df13 $MAIA_EmoAwareness <- rowMeans(select(df13 , starts_with("MAIA2_Emotional")))
df13 $MAIA_Noticing <- rowMeans(select(df13 , starts_with("MAIA2_Noticing")))
df13 $MAIA_BodyListening <- rowMeans(select(df13 , starts_with("MAIA2_BodyListening")))
df13 $MAIA_NotDistracting<- rowMeans(select(df13 , starts_with("MAIA2_NotDistracting")))
df13 $MAIA_NotWorrying<- rowMeans(select(df13 , starts_with("MAIA2_NotWorrying")))
df13 $MAIA_SelfReg<- rowMeans(select(df13 , starts_with("MAIA2_SelfRegulation")))
df13 $MAIA_Trusting<- rowMeans(select(df13 , starts_with("MAIA2_Trusting")))
...
# Save all
data <- list(df1a=df1a, df1b=df1b, df2=df2, df3=df3, df4=df4, df5=df5, df6=df6, df7a=df7a, df7b=df7b, df7c=df7c, df8a=df8a, df8b=df8b, df9=df9, df10=df10, df11 =df11 , df12 =df12 , df13  = df13 , df14 =df14 , df15  =df15)
...
Total N = `r nrow(df1a) + nrow(df1b) + nrow(df2) + nrow(df3) + nrow(df4) + nrow(df5) + nrow(df6) + nrow(df7a) + nrow(df7b) + nrow(df7c) + nrow(df8a) + nrow(df8b) + nrow(df9) + nrow(df10) + nrow(df11) + nrow(df12) + nrow(df13) + nrow(df14) + nrow(df15)`.
...
desc_total <- rbind(desc_df1a, desc_df1b, desc_df2, desc_df3, desc_df5, desc_df6, desc_df7a, desc_df7b, desc_df7c, desc_df8a, desc_df8b, desc_df9, desc_df10, desc_df11 , desc_df12 , desc_df13 , desc_df14 , desc_df15 )
...
table <- data.frame(
  Reference = c('Murphy et al., (2020)', '', '', 'Gaggero et al., (2021)', 'Campos et al., (2022)', 'Todd et al., (2022)', 'Arslanova et al., (2022)', 'Brand et al., (2022)', 'Brand et al., (2023)', '', '', '', 'Lin et al., (2023)', '', '', 'VonMohr et al., (2023)', 'Makowski et al., (2023a)', 'Makowski et al., (2023b)', 'Makowski et al., (2023c)', 'Makowski et al., (2024)', 'Poerio et al., (2024)', 'Poerio et al., unpublished', 'Total'),
  Sample = c('', 'Sample 1a', 'Sample 1b', 'Sample 2', 'Sample 3', 'Sample 4', 'Sample 5', 'Sample 6', '', 'Sample 7a', 'Sample 7b', 'Sample 7c', '', 'Sample 8a', 'Sample 8b', 'Sample 9', 'Sample 10', 'Sample 11', 'Sample 12', 'Sample 13', 'Sample 14', 'Sample 15', 'Sample 16'),   
  Language = c('', 'English', 'English' , 'English and Italian', 'Portuguese', 'English', 'English', 'German', '', 'German', 'German', 'German', '', 'Chinese', 'Chinese', 'English', 'English', 'English', 'English', 'English', 'English', 'English',''),
  N = c('', nrow(df1a), nrow(df1b), nrow(df2), nrow(df3), nrow(df4), nrow(df5_attention), nrow(df6),'', nrow(df7a), nrow(df7b), 802,'', nrow(df8a), nrow(df8b), nrow(df9), nrow(df10), nrow(df11), nrow(df12), nrow(df13), nrow(df14), nrow(df15), 32993),
  'Difference' = c('','', '', '', '', '', '', '', '', '', '', '', '', 'Collapsed "Itch" and "Tickling"', 'Collapsed "Itch" and "Tickling"', '', 'Analog scales. Does not include temperature, blood sugar and cough items.', 'Analog scales', 'Analog scales', '', '', '', ''),
  Age = c('', make_age(df1a$Age), make_age(df1b$Age), make_age(df2$Age), make_age(df3$Age), "48.6.6 ± 14.1*", make_age(df5_attention$Age), make_age(df6$Age),'', make_age(df7a$Age), make_age(df7b$Age), make_age(df7c$Age),'', make_age(df8a$Age), make_age(df8b$Age), make_age(df9$Age), make_age(df10$Age), make_age(df11$Age), make_age(df12$Age), make_age(df13$Age), make_age(df14$Age), make_age(df15$Age), '48.6 ± 13.1'),
  Range = c('', '18-69', '18-91', '18-58', '18-72', '18-92*', '18-73', '18-78', '', '18-79', '16-81', '18-72','', '16-60', '20-60', '18-93', '18-73', '17-76','18-50', '17, 87', '18-57', '18-60', '17-93'),
  Female_Percentage = c('', '69.4%', '70.1%', '60.3%', '59.6%', '50%*', '46.8%', '78.7%', '', '79.5%', '77.7%', '68.9%', '', '57.0%', '56.2%', '73.2%', '50.3%', '53.0%', '76%', '57.3%', '74.8%', '75.9%', '71.6%'),
...
dens10 <- estimate_density(select(df10, all_of(setdiff(vars, c("Taste", "Cough", "Blood_Sugar")))), method = "kernSmooth") |>
  mutate(Sample = "Sample 10",
         x = datawizard::rescale(x, to=c(1, 5), range=c(0, 1)))
dens11<- estimate_density(select(df11, all_of(vars)), method = "kernSmooth") |>
  mutate(Sample = "Sample 11",
         x = datawizard::rescale(x, to=c(1, 5), range=c(0, 1)))
dens12  <- estimate_density(select(df12, all_of(vars)), method = "kernSmooth") |>
    mutate(Sample = "Sample 12",
         x = datawizard::rescale(x, to=c(1, 5), range=c(0, 1)))
dens13  <- estimate_density(select(df13, all_of(vars)), method = "kernSmooth") |>
...
data6 <- normalize(select(df4, all_of(dens4$Parameter)), range = c(1, 5))
...
data_all <- rbind(
  data1a, data1b, data2, data3, data6, data5, data6, data7a, data7b, data7c,
  ...
  correlation::correlation(df, redundant = TRUE) |>
    ...
    scale_fill_gradientn(
      colors = c("white", "#FFF9C4", "#FFF59D", "#FFEB3B", "#FFCA28", "#FF9800", "#FF5722",
        "#F44336", "#E91E63", "#9C27B0", "#673AB7", "#512DA8",  "#3F51B5", 
        "#1976D2",  "#00ACC1","#00897B", "#00796B", "#31B192"),,
      na.value = "#31B192",
      limits = c(0, 0.8),
...
rez_ega <- make_ega(data1b, cols = c(
  "C1" = colors[["Heart"]], "C2" = colors[["Hungry"]], "C3" = colors[["Defecate"]], "C4" = colors[["Muscles"]], 
  "C5" = colors[["Cough"]], "C6" = colors[["Wind"]], "C7" = colors[["Bruise"]], "C8" = colors[["Blood Sugar"]]))
rez_efa <- make_efa(data1b, n = 4, cols = c("F1" = colors[["Wind"]], "F2" = colors[["Defecate"]], 
                                            "F3" = colors[["Heart"]], "F4" = colors[["Itch"]]))
...
rez_ega <- make_ega(data1af, cols = c(
  "C1" = colors[["Heart"]], "C2" = colors[["Hungry"]], "C3" = colors[["Defecate"]], "C4" = colors[["Muscles"]], 
  "C5" = colors[["Cough"]], "C6" = colors[["Wind"]], "C7" = colors[["Bruise"]], "C8" = colors[["Blood Sugar"]]))
rez_efa <- make_efa(data1bf, n = 4, cols = c("F1" = colors[["Wind"]], "F2" = colors[["Defecate"]], 
                                            "F3" = colors[["Heart"]], "F4" = colors[["Itch"]]))
...
rez_ega <- make_ega(data2, cols = c(
  "C1" = colors[["Heart"]], "C2" = colors[["Hungry"]], "C3" = colors[["Defecate"]], "C4" = colors[["Muscles"]], 
  "C5" = colors[["Cough"]], "C6" = colors[["Wind"]], "C7" = colors[["Bruise"]], "C8" = colors[["Blood Sugar"]]))
rez_efa <- make_efa(data2, n = 4, cols = c("F1" = colors[["Wind"]], "F2" = colors[["Defecate"]], 
                                            "F3" = colors[["Heart"]], "F4" = colors[["Itch"]]))
...
rez_ega <- make_ega(data2f, cols = c(
  "C1" = colors[["Heart"]], "C2" = colors[["Hungry"]], "C3" = colors[["Defecate"]], "C4" = colors[["Muscles"]], 
  "C5" = colors[["Cough"]], "C6" = colors[["Wind"]], "C7" = colors[["Bruise"]], "C8" = colors[["Blood Sugar"]]))
rez_efa <- make_efa(data2f, n = 4, cols = c("F1" = colors[["Wind"]], "F2" = colors[["Defecate"]], 
                                            "F3" = colors[["Heart"]], "F4" = colors[["Itch"]]))
...
rez_ega <- make_ega(data3, cols = c(
  "C1" = colors[["Heart"]], "C2" = colors[["Hungry"]], "C3" = colors[["Defecate"]], "C4" = colors[["Muscles"]], 
  "C5" = colors[["Cough"]], "C6" = colors[["Wind"]], "C7" = colors[["Bruise"]], "C8" = colors[["Blood Sugar"]]))
rez_efa <- make_efa(data3, n = 4, cols = c("F1" = colors[["Wind"]], "F2" = colors[["Defecate"]], 
                                            "F3" = colors[["Heart"]], "F4" = colors[["Itch"]]))
...
rez_ega <- make_ega(data3f, cols = c(
  "C1" = colors[["Heart"]], "C2" = colors[["Hungry"]], "C3" = colors[["Defecate"]], "C4" = colors[["Muscles"]], 
  "C5" = colors[["Cough"]], "C6" = colors[["Wind"]], "C7" = colors[["Bruise"]], "C8" = colors[["Blood Sugar"]]))
rez_efa <- make_efa(data3f, n = 4, cols = c("F1" = colors[["Wind"]], "F2" = colors[["Defecate"]], 
                                            "F3" = colors[["Heart"]], "F4" = colors[["Itch"]]))
rez_cf3  <- make_cfa(data3f)
...
rez_ega <- make_ega(data4, cols = c(
  "C1" = colors[["Heart"]], "C2" = colors[["Hungry"]], "C3" = colors[["Defecate"]], "C4" = colors[["Muscles"]], 
  "C5" = colors[["Cough"]], "C6" = colors[["Wind"]], "C7" = colors[["Bruise"]], "C8" = colors[["Blood Sugar"]]))
rez_efa <- make_efa(data4, n = 4, cols = c("F1" = colors[["Wind"]], "F2" = colors[["Defecate"]], 
                                            "F3" = colors[["Heart"]], "F4" = colors[["Itch"]]))
...
rez_ega <- make_ega(data4f, cols = c(
  "C1" = colors[["Heart"]], "C2" = colors[["Hungry"]], "C3" = colors[["Defecate"]], "C4" = colors[["Muscles"]], 
  "C5" = colors[["Cough"]], "C6" = colors[["Wind"]], "C7" = colors[["Bruise"]], "C8" = colors[["Blood Sugar"]]))
rez_efa <- make_efa(data4f, n = 4, cols = c("F1" = colors[["Wind"]], "F2" = colors[["Defecate"]], 
                                            "F3" = colors[["Heart"]], "F4" = colors[["Itch"]]))
...
rez_ega <- make_ega(data5, cols = c(
  "C1" = colors[["Heart"]], "C2" = colors[["Hungry"]], "C3" = colors[["Defecate"]], "C4" = colors[["Muscles"]], 
  "C5" = colors[["Cough"]], "C6" = colors[["Wind"]], "C7" = colors[["Bruise"]], "C8" = colors[["Blood Sugar"]]))
rez_efa <- make_efa(data5, n = 4, cols = c("F1" = colors[["Wind"]], "F2" = colors[["Defecate"]], 
                                            "F3" = colors[["Heart"]], "F4" = colors[["Itch"]]))
...
rez_ega <- make_ega(data5f, cols = c(
  "C1" = colors[["Heart"]], "C2" = colors[["Hungry"]], "C3" = colors[["Defecate"]], "C4" = colors[["Muscles"]], 
  "C5" = colors[["Cough"]], "C6" = colors[["Wind"]], "C7" = colors[["Bruise"]], "C8" = colors[["Blood Sugar"]]))
rez_efa <- make_efa(data5f, n = 4, cols = c("F1" = colors[["Wind"]], "F2" = colors[["Defecate"]], 
                                            "F3" = colors[["Heart"]], "F4" = colors[["Itch"]]))
...
rez_ega <- make_ega(data6, cols = c(
  "C1" = colors[["Heart"]], "C2" = colors[["Hungry"]], "C3" = colors[["Defecate"]], "C4" = colors[["Muscles"]], 
  "C5" = colors[["Cough"]], "C6" = colors[["Wind"]], "C7" = colors[["Bruise"]], "C8" = colors[["Blood Sugar"]]))
rez_efa <- make_efa(data6, n = 4, cols = c("F1" = colors[["Wind"]], "F2" = colors[["Defecate"]], 
                                            "F3" = colors[["Heart"]], "F4" = colors[["Itch"]]))
...
rez_ega <- make_ega(data6f, cols = c(
  "C1" = colors[["Heart"]], "C2" = colors[["Hungry"]], "C3" = colors[["Defecate"]], "C4" = colors[["Muscles"]], 
  "C5" = colors[["Cough"]], "C6" = colors[["Wind"]], "C7" = colors[["Bruise"]], "C8" = colors[["Blood Sugar"]]))
rez_efa <- make_efa(data6f, n = 4, cols = c("F1" = colors[["Wind"]], "F2" = colors[["Defecate"]], 
                                            "F3" = colors[["Heart"]], "F4" = colors[["Itch"]]))
rez_cf6  <- make_cfa(data6f)
...
rez_ega <- make_ega(data7a, cols = c(
  "C1" = colors[["Heart"]], "C2" = colors[["Hungry"]], "C3" = colors[["Defecate"]], "C4" = colors[["Muscles"]], 
  "C5" = colors[["Cough"]], "C6" = colors[["Wind"]], "C7" = colors[["Bruise"]], "C8" = colors[["Blood Sugar"]]))
rez_efa <- make_efa(data7a, n = 4, cols = c("F1" = colors[["Wind"]], "F2" = colors[["Defecate"]], 
                                            "F3" = colors[["Heart"]], "F4" = colors[["Itch"]]))
...
rez_ega <- make_ega(data7af, cols = c(
  "C1" = colors[["Heart"]], "C2" = colors[["Hungry"]], "C3" = colors[["Defecate"]], "C4" = colors[["Muscles"]], 
  "C5" = colors[["Cough"]], "C6" = colors[["Wind"]], "C7" = colors[["Bruise"]], "C8" = colors[["Blood Sugar"]]))
rez_efa <- make_efa(data7af, n = 4, cols = c("F1" = colors[["Wind"]], "F2" = colors[["Defecate"]], 
                                            "F3" = colors[["Heart"]], "F4" = colors[["Itch"]]))
...
rez_ega <- make_ega(data7b, cols = c(
  "C1" = colors[["Heart"]], "C2" = colors[["Hungry"]], "C3" = colors[["Defecate"]], "C4" = colors[["Muscles"]], 
  "C5" = colors[["Cough"]], "C6" = colors[["Wind"]], "C7" = colors[["Bruise"]], "C8" = colors[["Blood Sugar"]]))
rez_efa <- make_efa(data7b, n = 4, cols = c("F1" = colors[["Wind"]], "F2" = colors[["Defecate"]], 
                                            "F3" = colors[["Heart"]], "F4" = colors[["Itch"]]))
...
rez_ega <- make_ega(data7bf, cols = c(
  "C1" = colors[["Heart"]], "C2" = colors[["Hungry"]], "C3" = colors[["Defecate"]], "C4" = colors[["Muscles"]], 
  "C5" = colors[["Cough"]], "C6" = colors[["Wind"]], "C7" = colors[["Bruise"]], "C8" = colors[["Blood Sugar"]]))
rez_efa <- make_efa(data7bf, n = 4, cols = c("F1" = colors[["Wind"]], "F2" = colors[["Defecate"]], 
                                            "F3" = colors[["Heart"]], "F4" = colors[["Itch"]]))
rez_cf7b  <- make_cfa(data7bf)
...
rez_ega <- make_ega(data7c, cols = c(
  "C1" = colors[["Heart"]], "C2" = colors[["Hungry"]], "C3" = colors[["Defecate"]], "C4" = colors[["Muscles"]], 
  "C5" = colors[["Cough"]], "C6" = colors[["Wind"]], "C7" = colors[["Bruise"]], "C8" = colors[["Blood Sugar"]]))
rez_efa <- make_efa(data7c, n = 4, cols = c("F1" = colors[["Wind"]], "F2" = colors[["Defecate"]], 
                                            "F3" = colors[["Heart"]], "F4" = colors[["Itch"]]))
...
rez_ega <- make_ega(data7cf, cols = c(
  "C1" = colors[["Heart"]], "C2" = colors[["Hungry"]], "C3" = colors[["Defecate"]], "C4" = colors[["Muscles"]], 
  "C5" = colors[["Cough"]], "C6" = colors[["Wind"]], "C7" = colors[["Bruise"]], "C8" = colors[["Blood Sugar"]]))
rez_efa <- make_efa(data7cf, n = 4, cols = c("F1" = colors[["Wind"]], "F2" = colors[["Defecate"]], 
                                            "F3" = colors[["Heart"]], "F4" = colors[["Itch"]]))
rez_cf7c  <- make_cfa(data7cf)
...
rez_ega <- make_ega(data8a, cols = c(
  "C1" = colors[["Heart"]], "C2" = colors[["Hungry"]], "C3" = colors[["Defecate"]], "C4" = colors[["Muscles"]], 
  "C5" = colors[["Cough"]], "C6" = colors[["Wind"]], "C7" = colors[["Bruise"]], "C8" = colors[["Blood Sugar"]]))
rez_efa <- make_efa(data8a, n = 4, cols = c("F1" = colors[["Wind"]], "F2" = colors[["Defecate"]], 
                                            "F3" = colors[["Heart"]], "F4" = colors[["Itch"]]))
...
rez_ega <- make_ega(data8af, cols = c(
  "C1" = colors[["Heart"]], "C2" = colors[["Hungry"]], "C3" = colors[["Defecate"]], "C4" = colors[["Muscles"]], 
  "C5" = colors[["Cough"]], "C6" = colors[["Wind"]], "C7" = colors[["Bruise"]], "C8" = colors[["Blood Sugar"]]))
rez_efa <- make_efa(data8af, n = 4, cols = c("F1" = colors[["Wind"]], "F2" = colors[["Defecate"]], 
                                            "F3" = colors[["Heart"]], "F4" = colors[["Itch"]]))
rez_cf8a  <- make_cfa(data8af)
...
rez_ega <- make_ega(data8b, cols = c(
  "C1" = colors[["Heart"]], "C2" = colors[["Hungry"]], "C3" = colors[["Defecate"]], "C4" = colors[["Muscles"]], 
  "C5" = colors[["Cough"]], "C6" = colors[["Wind"]], "C7" = colors[["Bruise"]], "C8" = colors[["Blood Sugar"]]))
rez_efa <- make_efa(data8b, n = 4, cols = c("F1" = colors[["Wind"]], "F2" = colors[["Defecate"]], 
                                            "F3" = colors[["Heart"]], "F4" = colors[["Itch"]]))
...
rez_ega <- make_ega(data8bf, cols = c(
  "C1" = colors[["Heart"]], "C2" = colors[["Hungry"]], "C3" = colors[["Defecate"]], "C4" = colors[["Muscles"]], 
  "C5" = colors[["Cough"]], "C6" = colors[["Wind"]], "C7" = colors[["Bruise"]], "C8" = colors[["Blood Sugar"]]))
rez_efa <- make_efa(data8bf, n = 4, cols = c("F1" = colors[["Wind"]], "F2" = colors[["Defecate"]], 
                                            "F3" = colors[["Heart"]], "F4" = colors[["Itch"]]))
rez_cf8b  <- make_cfa(data8bf)
...
rez_ega <- make_ega(data9, cols = c(
  "C1" = colors[["Heart"]], "C2" = colors[["Hungry"]], "C3" = colors[["Defecate"]], "C4" = colors[["Muscles"]], 
  "C5" = colors[["Cough"]], "C6" = colors[["Wind"]], "C7" = colors[["Bruise"]], "C8" = colors[["Blood Sugar"]]))
rez_efa <- make_efa(data9, n = 4, cols = c("F1" = colors[["Wind"]], "F2" = colors[["Defecate"]], 
                                            "F3" = colors[["Heart"]], "F4" = colors[["Itch"]]))
...
rez_ega <- make_ega(data9f, cols = c(
  "C1" = colors[["Heart"]], "C2" = colors[["Hungry"]], "C3" = colors[["Defecate"]], "C4" = colors[["Muscles"]], 
  "C5" = colors[["Cough"]], "C6" = colors[["Wind"]], "C7" = colors[["Bruise"]], "C8" = colors[["Blood Sugar"]]))
rez_efa <- make_efa(data9f, n = 4, cols = c("F1" = colors[["Wind"]], "F2" = colors[["Defecate"]], 
                                            "F3" = colors[["Heart"]], "F4" = colors[["Itch"]]))
rez_cf9  <- make_cfa(data9f)
...
rez_ega <- make_ega(data10, cols = c(
  "C1" = colors[["Heart"]], "C2" = colors[["Hungry"]], "C3" = colors[["Defecate"]], "C4" = colors[["Muscles"]], 
  "C5" = colors[["Cough"]], "C6" = colors[["Wind"]], "C7" = colors[["Bruise"]], "C8" = colors[["Blood Sugar"]]))
rez_efa <- make_efa(data10, n = 4, cols = c("F1" = colors[["Wind"]], "F2" = colors[["Defecate"]], 
                                            "F3" = colors[["Heart"]], "F4" = colors[["Itch"]]))
...
rez_ega <- make_ega(data10f, cols = c(
  "C1" = colors[["Heart"]], "C2" = colors[["Hungry"]], "C3" = colors[["Defecate"]], "C4" = colors[["Muscles"]], 
  "C5" = colors[["Cough"]], "C6" = colors[["Wind"]], "C7" = colors[["Bruise"]], "C8" = colors[["Blood Sugar"]]))
rez_efa <- make_efa(data10f, n = 4, cols = c("F1" = colors[["Wind"]], "F2" = colors[["Defecate"]], 
                                            "F3" = colors[["Heart"]], "F4" = colors[["Itch"]]))
rez_cf10  <- make_cfa(data10f)
...
rez_ega <- make_ega(data11, cols = c(
  "C1" = colors[["Heart"]], "C2" = colors[["Hungry"]], "C3" = colors[["Defecate"]], "C4" = colors[["Muscles"]], 
  "C5" = colors[["Cough"]], "C6" = colors[["Wind"]], "C7" = colors[["Bruise"]], "C8" = colors[["Blood Sugar"]]))
rez_efa <- make_efa(data11, n = 4, cols = c("F1" = colors[["Wind"]], "F2" = colors[["Defecate"]], 
                                            "F3" = colors[["Heart"]], "F4" = colors[["Itch"]]))
...
rez_ega <- make_ega(data12, cols = c(
  "C1" = colors[["Heart"]], "C2" = colors[["Hungry"]], "C3" = colors[["Defecate"]], "C4" = colors[["Muscles"]], 
  "C5" = colors[["Cough"]], "C6" = colors[["Wind"]], "C7" = colors[["Bruise"]], "C8" = colors[["Blood Sugar"]]))
rez_efa <- make_efa(data12, n = 4, cols = c("F1" = colors[["Wind"]], "F2" = colors[["Defecate"]], 
                                            "F3" = colors[["Heart"]], "F4" = colors[["Itch"]]))
...
rez_ega <- make_ega(data8af, cols = c(
  "C1" = colors[["Heart"]], "C2" = colors[["Hungry"]], "C3" = colors[["Defecate"]], "C4" = colors[["Muscles"]], 
  "C5" = colors[["Cough"]], "C6" = colors[["Wind"]], "C7" = colors[["Bruise"]], "C8" = colors[["Blood Sugar"]]))
rez_efa <- make_efa(data12f, n = 4, cols = c("F1" = colors[["Wind"]], "F2" = colors[["Defecate"]], 
                                            "F3" = colors[["Heart"]], "F4" = colors[["Itch"]]))
rez_cf12  <- make_cfa(data12f)
...
rez_ega <- make_ega(data13, cols = c(
  "C1" = colors[["Heart"]], "C2" = colors[["Hungry"]], "C3" = colors[["Defecate"]], "C4" = colors[["Muscles"]], 
  "C5" = colors[["Cough"]], "C6" = colors[["Wind"]], "C7" = colors[["Bruise"]], "C8" = colors[["Blood Sugar"]]))
rez_efa <- make_efa(data13, n = 4, cols = c("F1" = colors[["Wind"]], "F2" = colors[["Defecate"]], 
                                            "F3" = colors[["Heart"]], "F4" = colors[["Itch"]]))
...
rez_ega <- make_ega(data13f, cols = c(
  "C1" = colors[["Heart"]], "C2" = colors[["Hungry"]], "C3" = colors[["Defecate"]], "C4" = colors[["Muscles"]], 
  "C5" = colors[["Cough"]], "C6" = colors[["Wind"]], "C7" = colors[["Bruise"]], "C8" = colors[["Blood Sugar"]]))
rez_efa <- make_efa(data13f, n = 4, cols = c("F1" = colors[["Wind"]], "F2" = colors[["Defecate"]], 
                                            "F3" = colors[["Heart"]], "F4" = colors[["Itch"]]))
rez_cf13  <- make_cfa(data13f)
...
rez_ega <- make_ega(data14, cols = c(
  "C1" = colors[["Heart"]], "C2" = colors[["Hungry"]], "C3" = colors[["Defecate"]], "C4" = colors[["Muscles"]], 
  "C5" = colors[["Cough"]], "C6" = colors[["Wind"]], "C7" = colors[["Bruise"]], "C8" = colors[["Blood Sugar"]]))
rez_efa <- make_efa(data14, n = 4, cols = c("F1" = colors[["Wind"]], "F2" = colors[["Defecate"]], 
                                            "F3" = colors[["Heart"]], "F4" = colors[["Itch"]]))
...
rez_ega <- make_ega(data15, cols = c(
  "C1" = colors[["Heart"]], "C2" = colors[["Hungry"]], "C3" = colors[["Defecate"]], "C4" = colors[["Muscles"]], 
  "C5" = colors[["Cough"]], "C6" = colors[["Wind"]], "C7" = colors[["Bruise"]], "C8" = colors[["Blood Sugar"]]))
rez_efa <- make_efa(data15, n = 4, cols = c("F1" = colors[["Wind"]], "F2" = colors[["Defecate"]], 
                                            "F3" = colors[["Heart"]], "F4" = colors[["Itch"]]))
...
rbind(
  ...
  mutate(rez_cfa1b$perf, Sample = "Sample 1b"),
  mutate(rez_cfa2$perf, Sample = "Sample 2"),
  mutate(rez_cfa3$perf, Sample = "Sample 3"),
  mutate(rez_cfa4$perf, Sample = "Sample 4"),
  mutate(rez_cfa5$perf, Sample = "Sample 5"),
  mutate(rez_cfa6$perf, Sample = "Sample 6"),
  mutate(rez_cfa7a$perf, Sample = "Sample 7a"),
  mutate(rez_cfa7b$perf, Sample = "Sample 7b"),
  mutate(rez_cfa7c$perf, Sample = "Sample 7c"),
  mutate(rez_cfa8a$perf, Sample = "Sample 8a"),
  mutate(rez_cfa8b$perf, Sample = "Sample 8b"),
  mutate(rez_cfa9$perf, Sample = "Sample 9"),
  mutate(rez_cfa10$perf, Sample = "Sample 10"),
  mutate(rez_cfa11$perf, Sample = "Sample 11"),
  mutate(rez_cfa12$perf, Sample = "Sample 12"),
  mutate(rez_cfa13$perf, Sample = "Sample 13"),
  mutate(rez_cfa14$perf, Sample = "Sample 14"),
  mutate(rez_cfa15$perf, Sample = "Sample 15")
) |> 
...
scores <- list(
  ...
  sample4 = add_empirical(data6, "Sample 4"),
