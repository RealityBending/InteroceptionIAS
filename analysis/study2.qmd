...
data <- rbind(
  ...
  # df7c
   make_cor(df7c, vars_intero, with = "MAIA_noticing", component="MAIA") |>
    mutate(var_Correlate = "Noticing"),
  make_cor(df7c, vars_intero, with = "MAIA_notdis", component="MAIA") |>
    mutate(var_Correlate = "Not Distracting"),
  make_cor(df7c, vars_intero, with = "MAIA_notwor", component="MAIA") |>
    mutate(var_Correlate = "Not Worrying"),
  make_cor(df7c, vars_intero, with = "MAIA_attreg", component="MAIA") |>
    mutate(var_Correlate = "Attention Regulation"),
  make_cor(df7c, vars_intero, with = "MAIA_emoaw", component="MAIA") |>
    mutate(var_Correlate = "Emotional Awareness"),
  make_cor(df7c, vars_intero, with = "MAIA_selfreg", component="MAIA") |>
    mutate(var_Correlate = "Self Regulation"),
  make_cor(df7c, vars_intero, with = "MAIA_bodlis", component="MAIA") |>
    mutate(var_Correlate = "Listening"),
  make_cor(df7c, vars_intero, with = "MAIA_trust", component="MAIA") |>
    mutate(var_Correlate = "Trusting"),
...
data <- rbind(
  data,
  make_cor(df2, vars_intero, with = "ICQ", component="ICQ") |>
    mutate(var_Correlate = "ICQ"),
  make_cor(df7c, vars_intero, with = "ICQ_sum", component="ICQ") |>
    mutate(var_Correlate = "ICQ")
)
...
- sample df7c does not have individual BPQ items 

```{r}
#| output: false

# BPQ 46 Items SV
df2_BPQ <- df2 |> select(contains("BPQ ")) 
df3_BPQ <- df3 |> select(contains("BPQ"), -contains("BPQ_"))



# BPQ 26 items - awareness subscale

df4_BPQ_A <- df4 |> select(contains("BPQ_"))

# BPQ 12 items VSF
df7a_BPQVSF <- df7b |> select(contains("BPQ_"), -BPQ_Sum)
df8a_BPQVSF <- df8a |> select(contains("BPQ"), -BPQ_A)
...
average_corr <- data |>
    summarise(mean_r = mean(r, na.rm = TRUE), .by=c("Component", "var_Correlate", "var_Interoception")) 

average_corr |>
  filter(Component == "Alexithymia") |>
  group_by(var_Correlate) |>
  summarise(min = min(mean_r),
             max = max(mean_r))

average_corr |>
  filter(Component == "BPQ") |>
  group_by(var_Correlate) |>
  summarise(min = min(mean_r),
             max = max(mean_r))
...
average_corr |>
  filter(Component == "MAIA") |>
  group_by(var_Correlate) |>
  summarise(min = min(mean_r),
             max = max(mean_r))

average_corr |>
  filter(Component == "ICQ") |>
  group_by(var_Correlate) |>
  summarise(min = min(mean_r),
             max = max(mean_r))

average_corr |>
  filter(Component == "Mood") |>
  group_by(var_Correlate) |>
  summarise(min = min(mean_r),
             max = max(mean_r))
...
average_corr |>
  filter(Component == "Schizotypic") |>
  group_by(var_Correlate) |>
  summarise(min = min(mean_r),
             max = max(mean_r))

average_corr |>
  filter(Component == "Autistic") |>
  group_by(var_Correlate) |>
  summarise(min = min(mean_r),
             max = max(mean_r))
...
average_corr |>
  filter(Component == "Personality") |>
  group_by(var_Correlate) |>
  summarise(min = min(mean_r),
             max = max(mean_r))

average_corr |>
  filter(Component == "World Beliefs") |>
  # group_by(var_Correlate) |>
  summarise(min = min(mean_r),
             max = max(mean_r))
