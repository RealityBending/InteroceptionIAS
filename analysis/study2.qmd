...
alldata <- list(df1a=df1a, df1b=df1b, df2=df2, df3=df3, df4=df4, 
                ...
                df12=df12, df14=df14, df15=df15, df16=df16)
```
...
pool_coef <- function(with = "Age", component = "Demographic", category = "Demographic", name = with, unequalrange=FALSE) {
  ...
  for(intero in vars_intero) {
    ...
    # Mixed models
    control1 <- glmmTMB::glmmTMBControl(parallel = list(n=4, autopar = TRUE))
    control2 <- glmmTMB::glmmTMBControl(optimizer=optim, optArgs=list(method="BFGS"), parallel = list(n=4, autopar = TRUE))
...
data <- rbind(
  data,
  pool_coef(with = "PHQ15_Sum", component="Mood", category = "Mood", name = "Somatic Concerns - PHQ-15",unequalrange=TRUE), 
  pool_coef(with = "PHQ9_Sum", component="Mood", category = "Mood", name = "Depression - PHQ-9", unequalrange=TRUE), 
  pool_coef(with = "BDI_Sum", component="Mood", category = "Mood", name = "Depression - BDI"), 
  pool_coef(with = "PHQ4_Depression", component="Mood", category = "Mood", name = "Depression - PHQ-4"),
  pool_coef(with = "DEP", component = "Mood", category = "Mood", name = "Depression - MFQ")
)
```
...
data <- rbind(
  data,
  pool_coef(with = "STAIT_Mean", component="Mood", category = "Mood", name = "Anxiety (STAI-T)", unequalrange=TRUE), 
  pool_coef(with = "PHQ4_Anxiety", component="Mood", category = "Mood", name = "Anxiety (PHQ-4)"),
  pool_coef(with = "GAD7_Sum", component="Mood", category = "Mood", name = "Anxiety (GAD-7)")
)
```
...
data <- rbind(
  data,
  pool_coef(with = "BPD", component="Mood", category = "Mood", name = "Boderline Personality")
)
```
...
p <- data |>
  mutate(sig = ifelse(p < .01, "p < .01", ifelse(p < .001, "p < .001", "N.S.")),
         ...
         Category = fct_relevel(
           Category, "General", "Interoception"),
         Component = fct_relevel(Component, "Personality", "Demographic", "Interoception", "MAIA")) |> 
  group_by(Component) |> 
...
p
ggsave("figures/Figure3.png", p, width=21*1, height=29.7*1, dpi=300, bg="white")
```
